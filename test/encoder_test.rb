...
class EncoderTest < Minitest::Test
  ...
  def test_check_for_braille_value_returns_array_of_braille
    ...
    assert_equal 1503, file_contents.check_for_braille_value.count
  end

  def test_split_braille_chars_returns_array_of_braille_chars
    file_contents = Encoder.new(data)

    assert_equal 9018, file_contents.split_braille_chars.count
   end

   def test_braille_pairs_returns_array_of_braille_pairs
    file_contents = Encoder.new(data)

    assert_equal 4509, file_contents.braille_pairs.count
   end
   ...
   def test_encoder_formats_braille_pairs_into_corresponding_levels
     ...
     assert_equal 19, file_contents.top.count
     assert_equal 19, file_contents.middle.count
     assert_equal 19, file_contents.bottom.count
   end
   ...
   def test_grid_values_have_correct_length
     ...
     assert_equal 160, file_contents.top[0].length
     assert_equal 160, file_contents.middle[0].length
     assert_equal 160, file_contents.bottom[0].length
   end
